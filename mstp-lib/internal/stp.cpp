...
// Fast convergence with priority changes (generated by Copilot) //

static BRIDGE_ID ExtractBpduRootId (const MSTP_BPDU* bpdu)
{
...
static bool IsObsoleteSelfRootForTree (STP_BRIDGE* bridge, unsigned int treeIndex, const BRIDGE_ID& announcedRoot, unsigned int timestamp)
{
    ...
    const BRIDGE_ID& myNow  = bridge->trees[treeIndex]->GetBridgeIdentifier();
    const BRIDGE_ID& myPrev = bridge->prevBridgeIdPerTree[treeIndex];

    unsigned int elapsedMs = (timestamp - bridge->lastPrioChangeTsPerTree[treeIndex]) * 1000U;
    if (elapsedMs > bridge->ignoreSelfOldRootMsPerTree[treeIndex])
        return false;
...
static bool IsObsoleteSelfRootBpdu (STP_BRIDGE* bridge,
									...
									const MSTP_BPDU* bpdu,
									unsigned int timestamp)
{
	// For the CIST:
	if (IsObsoleteSelfRootForTree(bridge, CIST_INDEX, ExtractBpduRootId(bpdu), timestamp)) {
		return true;
	}

	// For MSTIs:
	if (type == VALIDATED_BPDU_TYPE_MST) {
		unsigned short headerRemainder = (unsigned short) sizeof(MSTP_BPDU) - (unsigned short) offsetof(MSTP_BPDU, mstConfigId);
		unsigned short mstiCount = 0;
		if (bpdu->Version3Length >= headerRemainder) {
			mstiCount = (bpdu->Version3Length - headerRemainder) / (unsigned short) sizeof(MSTI_CONFIG_MESSAGE);
		}
		const MSTI_CONFIG_MESSAGE* mstis = (const MSTI_CONFIG_MESSAGE*) &bpdu[1];
		for (unsigned int mi = 0; mi < mstiCount; ++mi) {
			unsigned int treeIndex = mi + 1;
			if (treeIndex >= bridge->treeCount()) break;
			const BRIDGE_ID& announced = mstis[mi].RegionalRootId;
			if (IsObsoleteSelfRootForTree(bridge, treeIndex, announced, timestamp)) {
				return true;
			}
		}
	}
							  ...
							  unsigned int debugLogBufferSize)
{
	...
	bridge->prevBridgeIdPerTree = (BRIDGE_ID*) callbacks->allocAndZeroMemory ((1 + bridge->mstiCount) * sizeof(BRIDGE_ID));
	bridge->hasPrevBridgeIdPerTree = (bool*) callbacks->allocAndZeroMemory ((1 + bridge->mstiCount) * sizeof(bool));
	bridge->lastPrioChangeTsPerTree = (unsigned int*) callbacks->allocAndZeroMemory ((1 + bridge->mstiCount) * sizeof(unsigned int));
	bridge->ignoreSelfOldRootMsPerTree = (unsigned int*) callbacks->allocAndZeroMemory ((1 + bridge->mstiCount) * sizeof(unsigned int));
	assert(bridge->prevBridgeIdPerTree && bridge->hasPrevBridgeIdPerTree && bridge->lastPrioChangeTsPerTree && bridge->ignoreSelfOldRootMsPerTree);
...
void STP_DestroyBridge (STP_BRIDGE* bridge)
{
	...
	bridge->callbacks.freeMemory (bridge->prevBridgeIdPerTree);
	bridge->callbacks.freeMemory (bridge->hasPrevBridgeIdPerTree);
	bridge->callbacks.freeMemory (bridge->lastPrioChangeTsPerTree);
	bridge->callbacks.freeMemory (bridge->ignoreSelfOldRootMsPerTree);
	...
	if (bridge->started)
	{
		...
		else
		{
			...
			if (type != VALIDATED_BPDU_TYPE_UNKNOWN)
			{
				...
				if (IsObsoleteSelfRootBpdu(bridge, type, (const MSTP_BPDU*)bpdu, timestamp))
				{
					LOG(bridge, portIndex, -1, "{T}: Dropping stale self-root BPDU (old BridgeID still propagating).\r\n", timestamp);
					LOG(bridge, -1, -1, "------------------------------------\r\n");
					FLUSH_LOG(bridge);
					return;
				}
...
// Added modifications to get fast convergence with priority changes (12/02/2026).
void STP_SetBridgePriority (STP_BRIDGE* bridge, unsigned int treeIndex, unsigned short bridgePriority, unsigned int timestamp)
{
	...
	if (bid.GetPriorityWithoutMstid() != bridgePriority)
	{
		...
		bridge->prevBridgeIdPerTree[treeIndex] = bid;
		bridge->hasPrevBridgeIdPerTree[treeIndex] = true;
		bridge->lastPrioChangeTsPerTree[treeIndex] = timestamp;
		unsigned int maxAge = bridge->trees[CIST_INDEX]->BridgeTimes.MaxAge;
		bridge->ignoreSelfOldRootMsPerTree[treeIndex] = maxAge * 1000U;
